...
export default function ArticlePage() {
  ...
  return <><Head><title>{article.title} | Family Dental Clinic Siwan</title><meta name="description" content={article.summary}/></Head><main className={styles.page}>
    ...
    <article className={styles.article}><div className={styles.intro}><span>From the Doctors</span><h1>{article.title}</h1>{article.summary && <p>{article.summary}</p>}<strong>{article.author}</strong></div>
